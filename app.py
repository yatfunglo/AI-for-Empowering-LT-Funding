...
try:
    ...
    st.error("❌ API Key 設定有誤，請檢查 Secrets。")
    st.stop()
...
if prompt := st.chat_input("老師，有咩可以幫到你？"):
    ...
    with st.chat_message("assistant"):
        try:
            # 💡 終極修正：將 SYSTEM_PROMPT 同 prompt 合併，行最穩定的 v1 路徑
            ...
            if response.text:
                ...
                st.error("AI 暫時反應唔到，請重試。")

        except Exception as e:
            st.error("⚠️ 連線失敗，請點擊 Reboot App。")
            with st.expander("IT 組技術日誌"):
                st.write(str(e))
